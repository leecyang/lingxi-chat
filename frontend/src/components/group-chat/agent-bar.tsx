      ...
      <div className="max-w-4xl mx-auto flex items-center gap-4">
        ...
        <TooltipProvider delayDuration={0}>
          <div className="flex items-center gap-3">
            {availableAgents.filter(agent => agent.status === 'APPROVED' && agent.enabled).map((agent) => (
              <Tooltip key={agent.id}>
                <TooltipTrigger asChild>
                  <div className="relative cursor-pointer">
                    <Avatar className="w-9 h-9 border-2 border-transparent hover:border-primary transition-colors">
                      <AvatarImage src={agent.avatar} alt={agent.displayName} data-ai-hint={agent.dataAiHint} />
